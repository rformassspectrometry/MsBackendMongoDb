...
test_that("backendInitialize() loads spectraIds and sets internal slots", {
    ...
    expect_equal(backend@spectraIds, c("SP1", "SP2"))
    expect_equal(backend@id_map, c("SP1", "SP2"))
...
test_that("peaksData() returns list of matrices with correct columns", {
    ...
    dbcon$ms_spectrum_coll$insert(
                               data.frame(spectrum_id_ = c("SP1", "SP2"))
                           )
...
test_that("spectraNames works", {
    ...
    dbcon$ms_spectrum_coll$insert(
                               data.frame(spectrum_id_ = c("SP10", "SP20",
                                                           "SP30"))
                           )
    ...
    expect_equal(res, c("SP10", "SP20", "SP30"))
...
test_that("spectraNames<- throws an error", {
    ...
    dbcon$ms_spectrum_coll$insert(
                               data.frame(spectrum_id_ = c("10", "20", "30"))
                           )
...
test_that("extractByIndex works", {
    ...
    ## Insert metadata as character
    dbcon$ms_spectrum_coll$insert(
                               data.frame(spectrum_id_ = c("1", "2", "3"),
                                          msLevel = 2L, rtime =c(1.1,2,3))
                           )
    ## Insert peaks as character
    dbcon$ms_peaks_coll$insert(
                            data.frame(
                                spectrum_id_ = c("1", "2", "3"),
                                mz = I(list(c(100, 200, 201.3), c(150, 250),
                                            c(12.1, 14, 15.4, 19))),
                                intensity = I(list(c(10, 20, 39), c(15, 25),
                                                   c(120, 140, 150, 190)))
                            )
                        )
...
test_that("subsetting with numeric, logical, and character indices works", {
    ...
    ## Numeric subsetting
    be_num <- be[1:2]
    expect_equal(be_num@spectraIds, c("SP1", "SP2"))
    ...
    expect_equal(spdata_num$spectrum_id_, c("SP1", "SP2"))
    expect_equal(spdata_num$rtime, be$rtime[1:2])

    ## Logical subsetting
    be_log <- be[c(TRUE, FALSE, TRUE)]
    expect_equal(be_log@spectraIds, c("SP1", "SP3"))
...
test_that("reset restores the backend with two collections", {
    ...
    expect_equal(be_reset@spectraIds, c("SP1", "SP2", "SP3"))
    expect_equal(be_reset@id_map, c("SP1", "SP2", "SP3"))
